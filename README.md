...
**隔离加餐台**（`quarantine_feeding`）是隔离动物的专门通道（普通投喂页那条隔离拦截照旧留着）：
- 只能挑档案状态此刻仍是「隔离」的动物，提交当下状态已变就整张作废，绝不偷记进普通投喂流水；
- 饲料名必须能在该动物**最近一张**巡查备注里找到原文（提交时现查，开页后补的新巡查也认），
  对不上最新备注就提交失败；
- 同一只隔离动物一个日历日只能落一张加餐单，库上 `uk_qf_animal_day` 唯一约束兜底——
  两人几乎同时记，只认先成功的那张；后到的失败原因（含先成功那张的单号/饲养员）带回前端，
  失败那笔回滚不留痕，刷新不会冒出来。
...
5. **隔离加餐台**（`quarantine_feeding`）—— 隔离动物夜加餐：饲料名、克数、当班饲养员，
   认最近一张巡查备注、同一日只落一张
